...
runuser -l $SUDO_USER -c 'git clone git@github.com:groupthreads/cli.git /opt/groupthreads-cli/'
chown -R root:root /opt/groupthreads-cli/
chmod +x /opt/groupthreads-cli/groupthreads.py
runuser -l $SUDO_USER -c 'echo "alias groupthreads=\"python /opt/groupthreads-cli/groupthreads.py\"" | tee -a ~/.'"$SHELL_CONFIG"
